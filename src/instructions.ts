 ...
 */
export function priceDiscipline(): string {
...
- Search for the specific thing that would change a specific game ("Guardians Red Sox probable pitchers September 23", "Camden Yards wind forecast tonight"), not for news in general. A vague search gives you vague reasons, and vague reasons are already priced.
...
export function bettingDiscipline(tools: ToolNames = {}): string {
  return [priceDiscipline(), claimDiscipline(tools), stakeDiscipline()].join("\n\n");
}
